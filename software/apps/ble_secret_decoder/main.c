...
void ble_evt_adv_report(ble_evt_t const* p_ble_evt) {
  ...
  */
  ...
  // new address (in little endian)
  // C0:98:E5:4E:03:97
  int find_address[6] = {0xC0, 0x98, 0xE5, 0x4E, 0x03, 0x97};
  for(int i=0; i < 6; i++){
  ...
  if (flag == true){
    ...
    for (int i=0; i < adv_len; i++){
      ...
      for (int j=0; j < length; j++){
        i++;
        // getting type
        if (j == 0 && adv_buf[i+j] == 0xFF){
          // printf("\nManufacture data found!\n");
          // +1 for type, +2 as offset of useful bytes
          index = i+1+2;
        }
      }
    }
    ...
    // print message
    for(index; index<length; index++) printf("%s\r", adv_buf + index);
  }
